...
void command_VisionTracking::Execute()
{
  double turn = 0;
  double power = 0;
  if (m_vision->isVisionOnline() && m_vision->getBlocks(0.65, 1.35, (int)(frc::SmartDashboard::GetNumber("Alliance: ", 0))).size() > 0)
  {
    PixyBlock desiredBlock = m_vision->getDesiredBlock(m_vision->getBlocks(0.65, 1.35, (int)(frc::SmartDashboard::GetNumber("Alliance: ", 0))));
